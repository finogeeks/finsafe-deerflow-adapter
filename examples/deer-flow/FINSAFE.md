...
- Allowlist egress: set `host_capabilities.allowlist_supported: true` in `finsafe-daemon.yaml` **and** `network_mode: allowlist` in sandbox config; do not use `proxy_profiles` (S1 executor does not support `network.mode=proxy`). Sidecar image must be **v0.9.20+ rebuilt after [finsafe#138](https://github.com/Geeksfino/finsafe/pull/138)** so in-cell `curl` / Python `requests` work (loopback relay); run `docker compose pull finsafe-saas` before testing.
