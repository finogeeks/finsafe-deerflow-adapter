...
| Sidecar 镜像 | `ghcr.io/geeksfino/finsafe-saas:v0.9.18`（可 pin 其他 tag） |
...
| 内存 | `memory_max` | `"2G"` | 单 cell cgroup 上限；用 `G`/`M`/`MB`，勿用 `GiB`（provider 大小写不敏感拒绝 IEC 后缀，见 [finsafe-security-guide.md §8](finsafe-security-guide.md#8-升级迁移说明破坏性变更)） |
...
| ~~`filesystem_deny_write_globs`~~ | — | **已拒绝**：wrapper-YAML-only，SaaS `HighLevelPolicyV1` 路由会 `bad_request_unknown_field`；要阻止写入敏感路径请移出 `filesystem_read_write_paths` 或用 `policy_extensions` |
| ~~`filesystem_skip_default_deny_read`~~ | — | **已拒绝**：同上；FinSAFE 内置 deny-read 在 deny 模式下始终合并，无法经此 provider 关闭 |
| `policy_extensions` | `{}` | **浅合并**到 FinSAFE HighLevelPolicyV1（顶层 dict 一层合并，嵌套 dict 整体替换） |
...
### Provider 启动报 `FinsafePolicyConfigError: ... wrapper-YAML field`

`config.yaml` 用了 `filesystem_skip_default_deny_read` 或 `filesystem_deny_write_globs`——这两个是 wrapper-YAML-only 字段，SaaS `HighLevelPolicyV1` 路由会拒绝。删除该行；详见 [finsafe-security-guide.md §8](finsafe-security-guide.md#8-升级迁移说明破坏性变更)。

### Provider 启动报 `memory_max does not accept IEC binary suffixes`

`memory_max` 用了 `GiB`/`MiB`/`KiB`/`TiB`/`PiB`（大小写不敏感）。改为 `2G`/`512M`/`512MB`/纯字节数；详见 [finsafe-security-guide.md §8](finsafe-security-guide.md#8-升级迁移说明破坏性变更)。
